...
dz_max=float(config['dz_max'])
dz_step=float(config['dz_step'])
levels_dz=np.arange(dz_min,dz_max,dz_step)

fig,ax=plt.subplots(figsize = (xsize, ysize))
im=ax.set_aspect('equal')
im=ax.set_ylim(ymin0,ymax0)
im=ax.set_xlim(xmin0,xmax0)
im=ax.set_title("Sin-Generated Curve",size='35')
im=ax.set_xlabel("x(m)",size='30')
im=ax.set_ylabel("y(m)",size='30')
im=ax.contourf(xgrid,ygrid,dz,levels_dz,cmap='viridis')
...
ds,dn,dsi,dnj,rho_r,rho_s,rho_n,coss,sins,area=grid.dsdncal(nx,ny,ds,dn,dsi,dnj,rho_r,rho_s,rho_n,coss,sins,area,xgrid,ygrid,j_rep)
...
if j_east==0 and j_hdown<=1:
    h_down=uniform.down(nx,ny,dn,eta,qp,snm,hs0,slope) #下流端水位
...
if j_inih==0:
    ...
    hpos_c=uniform.h_uniform(hpos_c,c_area,vel_ave,nx,ny,dn,eta,qp,snm,hs0,h_down,hmin,slope,g)
elif j_inih==2:
    hpos_c=uniform.h_nonuni(hpos_c,c_area,vel_ave,e_slope,alf_f,eta,qp,spos_c,hs0,h_down,nx,ny,nym,ds,dn,snm,hmin,g)
...
# Velocity Contour and Velocity Vector
        ...
        cont=ax.contourf(xgrid[0:nx,:], ygrid[0:nx,:], hsx[0:nx,:],levels_depth, cmap='viridis') 
...
# Plot Sediment Transport
        ...
        qbx,qby,qb_node=uxvx.qbsqbn(qbx,qby,qbs_node,qbn_node,qb_node,qb_cell,qbs,qbn,nx,ny,coss,sins)
...
# Plot Bed Deformation
        if time>bstime:
            ...
            im=ax.set_ylim(ymin0,ymax0)
            im=ax.set_xlim(xmin0,xmax0)
            cont=ax.contourf(xgrid[0:nx,:], ygrid[0:nx,:], deta_node[0:nx,:],levels_dz) 
            vect=ax.quiver(xgrid[0:nx:iskip,0:nx:jskip], ygrid[0:nx:iskip,0:nx:jskip], ux[0:nx:iskip,0:nx:jskip], vx[0:nx:iskip,0:nx:jskip], \
           scale=jscale, color='purple', width=0.001,headwidth=1)
            divider = mpl_toolkits.axes_grid1.make_axes_locatable(ax)
            cax = divider.append_axes('right', '5%', pad='3%')
            cb=fig.colorbar(cont, cax=cax)
            cb.set_label('Elevation Change(m)', size=30)
#            plt.subplots_adjust(left=xset1, right=xset2, bottom=yset1, top=yset2)
            for i in np.arange(1,nx+1):
                for j in np.arange(1,ny+1):
                    if ijh[i,j]>=0.1:
